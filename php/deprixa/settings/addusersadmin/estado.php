...
$stmt = $con->prepare("SELECT estado FROM manager_admin WHERE cid=?");
...
if($estado['estado'] == '1'){
	...
	$updateStmt = $con->prepare("UPDATE manager_admin SET estado='0' WHERE cid=?");
} else {
	// Cambiamos el estado a activo
	$updateStmt = $con->prepare("UPDATE manager_admin SET estado='1' WHERE cid=?");
}
