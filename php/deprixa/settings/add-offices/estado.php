...
$stmt = $con->prepare("SELECT estado FROM offices WHERE id=?");
...
if($estado['estado'] == '1'){
	...
	$updateStmt = $con->prepare("UPDATE offices SET estado='0' WHERE id=?");
} else {
	// Cambiamos el estado a activo
	$updateStmt = $con->prepare("UPDATE offices SET estado='1' WHERE id=?");
}
