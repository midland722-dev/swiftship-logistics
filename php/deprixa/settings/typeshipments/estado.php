...
$stmt = $con->prepare("SELECT estado FROM type_shipments WHERE id=?");
...
if($estado['estado'] == '1'){
	...
	$updateStmt = $con->prepare("UPDATE type_shipments SET estado='0' WHERE id=?");
} else {
	// Cambiamos el estado a activo
	$updateStmt = $con->prepare("UPDATE type_shipments SET estado='1' WHERE id=?");
}
